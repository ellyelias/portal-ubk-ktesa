...
export default function CoretanManager({email,role}:{email:string;role:string}){
 const[posts,setPosts]=useState<Post[]>([]);const[editing,setEditing]=useState<Post|null>(null);const[open,setOpen]=useState(false);const[busy,setBusy]=useState(false);const[error,setError]=useState("");
 async function load(){const r=await fetch("/kaunselor/coretan/api",{cache:"no-store"});const d=await r.json();if(r.ok)setPosts(d.posts||[]);else setError(d.error||"Gagal memuatkan coretan.");}
 useEffect(()=>{load()},[]);
 function newPost(){setEditing(null);setOpen(true);setError("")} function edit(post:Post){setEditing(post);setOpen(true);setError("")}
 async function submit(form:HTMLFormElement,status:string){setBusy(true);setError("");const fd=new FormData(form);if(editing)fd.set("id",String(editing.id));fd.set("status",status);
 const r=await fetch("/kaunselor/coretan/api",{method:"POST",body:fd});const d=await r.json();setBusy(false);if(!r.ok){setError(d.error||"Gagal menyimpan.");return}
 setOpen(false);setEditing(null);await load();}
 async function save(e:FormEvent<HTMLFormElement>){e.preventDefault();await submit(e.currentTarget,"published")}
 async function remove(id:number){if(!confirm("Padam coretan ini?"))return;const r=await fetch(`/kaunselor/coretan/api?id=${id}`,{method:"DELETE"});const d=await r.json();if(!r.ok){setError(d.error||"Gagal memadam.");return}await load();}
 ...
 {role==="kaunselor"&&<button className="coretan-new-btn" onClick={newPost}><Plus/> Coretan baharu</button>}</div>{error&&<p className="form-error">{error}</p>}
 ...
 <button className="publish" disabled={busy} type="submit"><Megaphone/> {editing?.status==="published"?"Kemas kini":"Terbitkan"}</button></footer></form>}
