...
export default function VisionPage(){
 ...
 const download=()=>{const canvas=document.createElement("canvas");canvas.width=1200;canvas.height=1500;const ctx=canvas.getContext("2d")!;const render=()=>{
  ...
  function finish(){ctx.fillStyle=u.deep+"91";ctx.fillRect(0,0,1200,1500);ctx.strokeStyle=u.accent;ctx.lineWidth=3;ctx.strokeRect(54,54,1092,1392);
 ...
 return <main className="vision-page"><div className="vision-shell"><Link className="back-link" href="/"><ArrowLeft/> Kembali ke portal</Link>
  ...
  <section className="vision-preview-wrap"><h2><Eye/> {lang==="bm"?"Vision Board Saya":"My Vision Board"}</h2><div className="vision-poster" style={{"--poster-deep":u.deep,"--poster-accent":u.accent,backgroundImage:`linear-gradient(${u.deep}82,${u.deep}a1),url(${u.bg})`} as React.CSSProperties}>
