...
import HomeCoretan from "./HomeCoretan";
...
export default function Home(){
 ...
 const [stats,setStats]=useState({visits:0,vision_downloads:0});
 useEffect(()=>{
  (async()=>{
   try{
    const firstVisit=!localStorage.getItem("ubk_visited");
    if(firstVisit){
     localStorage.setItem("ubk_visited","1");
     const r=await fetch("/api/stats",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({key:"visits"})});
 ...
 return <main>
  ...
  <div className="college-banner"><img src="/header-banner-ubk.jpg" alt="Unit Bimbingan dan Kaunseling KTESA Portal — Kolej Tingkatan Enam (Prauniversiti) Shah Alam" width={2048} height={768}/></div>
  ...
  <section style={{padding:"32px 20px",background:"#fff",borderTop:"1px solid #dce3e3",borderBottom:"1px solid #dce3e3"}}><div className="shell" style={{display:"flex",gap:20,flexWrap:"wrap",justifyContent:"center"}}>
   <div style={{flex:"1 1 220px",maxWidth:280,textAlign:"center",padding:"18px",borderRadius:12,background:"#f5f4ef"}}><strong style={{display:"block",fontSize:30,fontWeight:800,color:"#102f46"}}>{stats.visits.toLocaleString("ms-MY")}</strong><span style={{fontSize:13,color:"#677783"}}>Pengguna telah melawat portal ini</span></div>
   <div style={{flex:"1 1 220px",maxWidth:280,textAlign:"center",padding:"18px",borderRadius:12,background:"#f5f4ef"}}><strong style={{display:"block",fontSize:30,fontWeight:800,color:"#177d78"}}>{stats.vision_downloads.toLocaleString("ms-MY")}</strong><span style={{fontSize:13,color:"#677783"}}>Vision Board telah dimuat turun</span></div>
  </div></section>
